...
css: ## Compile scss/ -> css/luna.css (readable output)
	$(SASS) $(SCSS_SRC):$(CSS_OUT) --no-source-map --no-charset --style=expanded

css-watch: ## Recompile on every save while you work
	$(SASS) --watch $(SCSS_SRC):$(CSS_OUT) --no-source-map --no-charset --style=expanded

css-min: ## Compile minified (optional)
	$(SASS) $(SCSS_SRC):$(CSS_OUT) --no-source-map --no-charset --style=compressed
