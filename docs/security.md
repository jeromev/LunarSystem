...
| **Vendored libs** | Maintenance | The only in-tree vendored library left is **semsol/arc2 3.1.0** (RDF/SPARQL), kept vendored on purpose: it carries local PHP-8 patches the upstream lacks (stock 3.1.0 fatals on `?output=n3`), so it is not a drop-in Composer package — see [`luna.lib/arc/VENDOR.txt`](../luna/luna.lib/arc/VENDOR.txt). The Composer dependencies are HTMLPurifier (input sanitiser) and league/commonmark (Markdown→HTML renderer), both committed under `vendor/`; logging, input sanitisation and caching are otherwise handled in-tree. |

## Security weaknesses

| Issue | Severity | Detail | Mitigation |
|---|---|---|---|
| **Legacy MD5 password hashes** | Medium | Pre-existing `luna_users.password` rows are bare `md5()` hashes (trivially crackable / rainbow-table-vulnerable). New and changed passwords use bcrypt (`password_hash`), and any surviving MD5 hash is transparently re-hashed to bcrypt on the owner's next successful login (`password_is_legacy()` / `password_needs_rehash()`, since 0.7.2). | Don't reuse real passwords; log every legacy admin in once to force the upgrade. |
| **Session ID in URL** | — | Fixed: cookie-only sessions. `session.use_trans_sid = 0` with `use_only_cookies = 1` and `use_strict_mode = 1` ([luna.php:33-35](../luna/luna.php#L33)), so the SID is never propagated through URLs and an attacker-supplied SID is rejected. | — (closed in 0.7.1 / 0.7.3). |
| **Weak default admin** | Medium | Seed admin is `admin@lunarsystem.local` / `luna`. | Change immediately after install. |
| **Sanitisation stack** | Medium | Input HTML cleaning uses **HTMLPurifier**, an allowlist DOM parser. Markdown page content is the one exemption — it is rendered with **league/commonmark** (raw HTML escaped, unsafe links dropped) rather than purified, since purifying would corrupt the source. SQL is escaped via PDO `quote()` (`lunaDB::quote`); SPARQL via `sparql_literal`/`rdf_uri` (audited injection-safe). Modern, but coverage should still not be assumed complete against every technique. | Audit before any untrusted exposure. |
...
**Status** is current as of **0.8.47-alpha** (✅ fixed, ◐ partially fixed, ⬜ open).
...
|---|---|---|---|---|
| **No CSRF protection anywhere** | High | ✅ fixed | A per-session synchroniser token is embedded in every state-changing form and verified centrally (POST-only `hash_equals`) in the dispatch before any handler runs; `?purge` is POST-only too. | dispatch [luna.php:494](../luna/luna.php#L494) (0.7.4) |
...
| **Stored content rendered unescaped** | Medium | ◐ partial | Page content is authored as **Markdown** and stored as source (`luna:content` in `luna_texts`). On the HTML view it is rendered to HTML (the transient `ui:content`) and emitted with `disable-output-escaping="yes"`. Safety rests on the **CommonMark converter** (`lunaTools::markdown()`, league/commonmark, 0.8.48): raw HTML in the source is **escaped** (`html_input=escape`) and unsafe URI schemes (`javascript:`, `data:`, …) are dropped (`allow_unsafe_links=false`), so `<script>`, `on*` handlers and `javascript:` URIs cannot reach the output *by construction*. The Markdown source itself is never disable-output-escaped (only the converted `ui:content` is) and is XML-escaped when echoed back into the edit textarea. The content field is exempt from the global **HTMLPurifier** pass (purifying would reflow/strip the Markdown source); every other input still goes through it. | [luna.default.html.xsl:33](../luna/luna.xsl/luna.html.xsl/luna.default.html.xsl#L33) (+ `luna.root`); [lunaTools::markdown()](../luna/luna.classes/luna.tools.class.php) |
